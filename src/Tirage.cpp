...
Tirage::Tirage() :
		allPersonnes(Tools::getInstance()->sizeMax), flagNouveauTour(false), flagPresAjout(false), maxNameAffLength(0), nbPersonnes(
				0), nomFichierAdd(""), maxIndice(0), nbTentatives(0) {
...
bool Tirage::rec_appar4(vector<Personne*> &vp, const array<int, 3> &dec, vector<Match*> &newmatch, int *mi, bool fl2,
	...
	flagcout
			&& cout << "rec_appar4 dec=" << tools->to_string(dec) << "prof=" << newmatch.size() << " vp.size()="
					<< vp.size() << endl;
